import { OffsetCoord, evenVerticalCoords, evenHorizontalCoords, type EvenVerticalCoordSystem } from '../../src/hex/Coords'
import { RectangularHexGrid, CompositeHexGrid, SparseHexGrid } from '../../src/hex/Grids'

test('HexGridOverlay Constructor', () => {
  const grid = CompositeHexGrid.overlay([RectangularHexGrid.generate(evenVerticalCoords, [-2, 1], [0, 4], (c, r) => `(${c},${r})`)])
...
test('HexGridOverlay Constructor underlying exists', () => {
  const t1 = (): void => {
    const x = CompositeHexGrid.overlay<number, EvenVerticalCoordSystem>([])
    console.log(x)
  }
...
test('HexGridOverlay Iterator', () => {
  const iter = CompositeHexGrid.overlay([RectangularHexGrid.generate0(evenVerticalCoords, 1, 1, (c, r) => `(${c},${r})`)])[Symbol.iterator]()
...
test('HexGridOverlay allHexAt', () => {
  const grid = CompositeHexGrid.overlay([RectangularHexGrid.generate0(evenVerticalCoords, 1, 1, (c, r) => `(${c},${r})`)])
  grid.set(new OffsetCoord(1, 1), 'b')
  expect(grid.allHexAt(new OffsetCoord(1, 1))).toStrictEqual(['b', '(1,1)'])
})

test('HexGridOverlay Set', () => {
  const underlying = RectangularHexGrid.generate0(evenVerticalCoords, 1, 1, (c, r) => `(${c},${r})`)
  const grid = CompositeHexGrid.overlay([underlying])
...
test('CompositeHexGrids requirements', () => {
  const t5 = (): void => {
    const x = new CompositeHexGrid([RectangularHexGrid.generate0(evenVerticalCoords, 4, 4, (c, r) => `(${c},${r})`)])
    console.log(x)
  }
...
test('CompositeHexGrids constructor', () => {
  const grid = new CompositeHexGrid([new SparseHexGrid(evenHorizontalCoords, new Map([[OffsetCoord.zero, 'a'], [new OffsetCoord(1, 1), 'b']])),
