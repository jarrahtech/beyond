...
test('SparseHexGrid Constructor', () => {
  const grid = new SparseHexGrid(oddVerticalCoords, new Map([[OffsetCoord.zero, 'a'], [new OffsetCoord(1, 1), 'b']]))
...
test('SparseHexGrid Iterator', () => {
  const iter = new SparseHexGrid(evenHorizontalCoords, new Map([[OffsetCoord.zero, 'a'], [new OffsetCoord(1, 1), 'b'], [new OffsetCoord(0, 1), 'c']]))[Symbol.iterator]()
...
test('SparseHexGrid Set', () => {
  const grid = new SparseHexGrid(evenHorizontalCoords, new Map([[OffsetCoord.zero, 'a'], [new OffsetCoord(1, 1), 'b']]))
  expect(grid.hexAt(new OffsetCoord(1, 1))).toBe('b')
...
test('SparseHexGrid clear', () => {
  const grid = SparseHexGrid.empty(oddVerticalCoords)
