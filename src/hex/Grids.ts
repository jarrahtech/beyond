import { defined, Opt, flatmap } from "../util/Opt";
import { Direction } from "./Common";
import { CoordSystem, OffsetCoord, CubeCoord }  from "./Coords";
...
export abstract class HexGrid<H, C extends CoordSystem> implements Iterable<[OffsetCoord, H]> {
  ...
  distance(c1: OffsetCoord, c2: OffsetCoord) { return this.coords.distance(c1, c2); }
  ...
  rangeMap<T>(center: CubeCoord, distance: number, f: (c: OffsetCoord) => Opt<T>): T[] {
    ...
    for (let q = -distance; q <= distance; q++) {
      for ( let r = Math.max(-distance, -q - distance); r <= Math.min(distance, -q + distance); r++) {   
        let h = f(this.coords.toOffset(center.add(new CubeCoord(q, r, -q - r))));
...
export class SparseHexGrid<H, C extends CoordSystem> extends HexGrid<H, C> {
  ...
  private hexes: Map<string, [OffsetCoord, H]> = new Map();
...
export class RectangularHexGrid<H, C extends CoordSystem> extends HexGrid<H, C> {
  ...
  constructor (coords: C, public colRange: [number, number], public rowRange: [number, number], private hexes: H[][]) {
    super(coords);
    if (colRange[0] > colRange[1]) throw `column range reversed: [${colRange[0]},${colRange[1]}]`;
    if (rowRange[0] > rowRange[1]) throw `row range reversed: [${rowRange[0]},${rowRange[1]}]`;
    this.numColumns = this.colRange[1] - this.colRange[0] + 1
    this.numRows = this.rowRange[1] - this.rowRange[0] + 1;
    if (hexes.length != this.numColumns) throw `grid not rectangular: width=${this.numColumns}`;
    if (!hexes.every(r => r.length == this.numRows)) throw `grid not rectangular: height=${this.numRows}`;
  }
  ...
  has(pos: OffsetCoord) { return pos.col >= this.colRange[0] && pos.col <= this.colRange[1] && pos.row >= this.rowRange[0] && pos.row <= this.rowRange[1]; }
...
export class CompositeHexGrid<H, C extends CoordSystem> extends HexGrid<H, C> {
  ...
  static overlay<H, C extends CoordSystem>(underlying: HexGrid<H, C>[]): CompositeHexGrid<H, C> { 
    if (underlying.length < 1) throw "overlay grids.length<1";
    let newArr = underlying.slice();
    newArr.unshift(SparseHexGrid.empty(underlying[0].coords));
    return new CompositeHexGrid(newArr)
  }
  constructor (private grids: HexGrid<H, C>[]) { 
    if (grids.length < 2) throw "grids.length<2";
